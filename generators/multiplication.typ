...
// 汎用的な引き算問題生成関数
#let generate-multiplication-problems(start, addend: 1, problems-per-sheet: 10) = {
  let problems = ()
  for i in range(problems-per-sheet) {
    let num = start + i
    problems.push(make-multiplication(num, addend))
  }
  problems
}
...
// 汎用的な引き算問題生成関数
#let create-multiplication-sheet(
  sheet-number,
  addend: 1,
  ...
  let problems = generate-multiplication-problems(start, addend: addend, problems-per-sheet: problems-per-sheet)  
