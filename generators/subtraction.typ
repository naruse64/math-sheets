...
// 汎用的な引き算問題生成関数
#let generate-subtraction-problems(start, addend: 1, problems-per-sheet: 10) = {
  let problems = ()
  for i in range(problems-per-sheet) {
    let num = start + i
    problems.push(make-subtraction(num, addend))
  }
  problems
}
...
#let create-subtraction-sheet(
  ...
  let problems = generate-subtraction-problems(start, addend: addend, problems-per-sheet: problems-per-sheet)  
